...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO register_student (firstname, lastname, username, email, phoneno, password, activation_token, p) VALUES ('$firstname', '$lastname', '$username', '$email', '$phoneno', '$hashedPassword', '$token','$p')";
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        try {
            ...
            $mail->addAddress($email, $firstname); // Student's email and name

            // Email content
            $mail->isHTML(true);
            $mail->Subject = 'Student Account Activation';
            $activation_link = "http://127.0.0.1/CodeWarriors/templates/activate_student.php?token=$token"; // Replace with your activation script URL
            ...
            // Success message
            echo json_encode(['status' => 'success', 'message' => 'Student account registered successfully! Activation email sent.']);
        } catch (Exception $e) {
            // Error message
